...
explicit **Apply / Restore** action. Named hosted custom profiles retain
advanced analytical settings only in session memory; the allowlist excludes
supplied text, bibliographic metadata, pronunciation overrides, completed
results, and exports.
...
analyses. Set rows reuse the established pairwise metric extractors and
calculate equal-poem numeric means plus poem-level population standard
