...
The explicit tracked exceptions include the redistributable public-domain
VerseMap reference corpus under `VerseMap_Reference_Corpus/`, the CC BY 4.0
Open English WordNet dictionary resource under `open_english_wordnet/`, and the
learner-only Word documents under `training/`. The training package excludes
evaluator answer keys, scoring rubrics, certificates, and private instructional
source files.
...
does not download separately installed research files automatically. The Open
English WordNet archive is already packaged for offline Lexicon Explorer
...
as affective evidence or a poem-level scoring source. Other private-cloud
resources remain governed by their own documented licenses and deployment
terms.
